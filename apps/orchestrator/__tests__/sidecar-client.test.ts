...
describe("sidecar client", () => {
  ...
  it("extractFile sends path and returns result", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ filename: "doc.pdf", content: "extracted text" }),
    })
    ...
    expect(mockFetch).toHaveBeenCalledWith("http://localhost:8000/extract", {
      method: "POST",
  ...
  it("extractUrl sends url and returns result", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ filename: "page.html", content: "web content" }),
    })
